...
def fetch_data():
    ...
    rows = cursor.fetchall()  # Mengambil semua data
    conn.close()
    return rows
...
def update():
    try:
        ...
        # Menghitung prediksi fakultas dan memperbarui database
        prediksi = calculate_prediction(biologi, fisika, inggris)
...
def populate_table():
    ...
    for row in fetch_data():
        tree.insert('', 'end', values=row)  # Memasukkan data baru ke tabel

# Fungsi untuk mengisi form dari tabel yang dipilih
...
#Pengaturan posisi isi tabel di tengah
for col in columns:
...
# Event binding untuk tabel
tree.bind('<ButtonRelease-1>', fill_inputs_from_table)

# Memuat data awal di tabel
populate_table()
